 ...
 *
 * @test 1.29 01/03/12
 * @bug 4031438 4058973 4074764 4094906 4104976 4105380 4106659 4106660 4106661 4111739 4112104
 *     4113018 4114739 4114743 4116444 4118592 4118594 4120552 4142938 4169959 4232154 4293229
...
public class MessageRegressionTest extends CoreTestFmwk {
    ...
    public void Test4074764() {
        String[] pattern = {
            "Message without param", "Message with param:{0}", "Longer Message with param {0}"
        };
        ...
        try {
            ...
            if (!tempBuffer.equals("Message without param"))
                errln(
                        "Formatted with arguments > substitution failed. result = "
                                + tempBuffer.toString());
    ...
    public void Test4031438() {
        ...
        try {
            ...
            if (!tempBuffer.equals(
                    "Impossible {1} has occurred -- status code is 7 and message is {2}."))
            ...
            tempBuffer = messageFormatter.format(null);
            if (!tempBuffer.equals(
                    "Impossible {1} has occurred -- status code is {0} and message is {2}."))
    ...
    public void Test4052223() {
        ...
        if (pos.getErrorIndex() == -1) errln("Bug 4052223 failed : parsing string " + str);
        pos.setErrorIndex(4);
    ...
    public void Test4106660() {
        ...
        if (!str.equals("Two")) errln("format(" + d + ") = " + cf.format(d));
    }
    ...
    public void Test4111739() {
        ...
        if (!format1.equals(format2)) {
            errln(
                    "MessageFormats before and after serialization are not"
                            + " equal\nformat1 = "
                            + format1
                            + "("
                            + format1.toPattern()
            ...
            try {
                ...
                if (array != null) {
                    ...
                    for (int j = 0; j < array.length; j++) {
                        if (array[j] != null) err("\"" + array[j].toString() + "\"");
                        else log("null");
    ...
    public void Test4094906() {
        ...
        if (!fmt.toPattern()
                .startsWith(
                        "-\u221E<are negative|0.0<are no or fraction|1.0#is one|1.0<is 1+|\u221E<are many."))
    ...
    public void Test4118594() {
        ...
        if (!objs[0].toString().equals("z")) errln("argument0: \"" + objs[0] + "\"");
        ...
        if (!result.equals("3.14, 3.1")) errln("result = " + result);
    ...
    public void Test4120552() {
        ...
        for (int i = 0; i < texts.length; i++) {
            ...
            if (objs == null) {
                ...
                if (pp.getErrorIndex() == -1) errln("Incorrect error index: " + pp.getErrorIndex());
            } else {
    ...
    /**
     * @bug 4142938 MessageFormat handles single quotes in pattern wrong. This is actually a problem
     *     in ChoiceFormat; it doesn't understand single quotes.
     */
    @Test
    public void Test4142938() {
        String pat =
                "''Vous'' {0,choice,0#n''|1#}avez s\u00E9lectionne\u00E9 "
    ...
    /**
     * @bug 4142938 Test the applyPattern and toPattern handling of single quotes by ChoiceFormat.
     *     (This is in here because this was a bug reported against MessageFormat.) The single quote
     *     is used to quote the pattern characters '|', '#', '<', and '\u2264'. Two quotes in a row
     *     is a quote literal.
     */
    @Test
    public void TestChoicePatternQuote() {
        ...
        for (int i = 0; i < DATA.length; i += 3) {
            try {
                ...
                else logln("Ok: Pattern \"" + DATA[i] + "\" x toPattern -> \"" + pat + '"');
            } catch (IllegalArgumentException e) {
    ...
    /**
     * @bug 4112104 MessageFormat.equals(null) throws a NullPointerException. The JLS states that it
     *     should return false.
     */
    @Test
    ...
    /**
     * @bug 4169959 MessageFormat does not format null objects. CANNOT REPRODUCE THIS BUG.
     */
    @Test
    ...
    public void test4293229() {
        ...
        if (!result.equals(expected)) {
            throw new RuntimeException(
                    "wrong format result - expected \"" + expected + "\", got \"" + result + "\"");
        }
    }
    ...
    public void testBugTestsWithNamesArguments() {
        ...
        { // Taken from Test4031438().
            String pattern1 =
                    "Impossible {arg1} has occurred -- status code is {arg0} and message is {arg2}.";
            String pattern2 =
                    "Double '' Quotes {ARG_ZERO} test and quoted '{ARG_ONE}' test plus 'other {ARG_TWO} stuff'.";
            ...
            try {
                ...
                if (!tempBuffer.equals(
                        "Impossible {arg1} has occurred -- status code is 7 and message is {arg2}."))
                    errln("Tests arguments < substitution failed");
                logln("Formatted with 7 : " + tempBuffer);
                ParsePosition status = new ParsePosition(0);
                ...
                for (String key : objs.keySet()) {
                    if (objs.get(key) != null
                            && !objs.get(key).toString().equals(paramsMap.get(key).toString())) {
                        errln(
                ...
                if (!tempBuffer.equals(
                        "Double ' Quotes 7 test and quoted {ARG_ONE} test plus 'other {ARG_TWO} stuff'."))
                    errln("quote format test (w/ params) failed.");
                logln("Formatted with params : " + tempBuffer);
                tempBuffer = messageFormatter.format(null);
                if (!tempBuffer.equals(
        ...
        { // Taken from Test4052223().
            ...
            MessageFormat fmt =
                    new MessageFormat(
                            "There are {numberOfApples} apples growing on the {whatKindOfTree} tree.");
            String str = new String("There is one apple growing on the peach tree.");
            ...
            if (pos.getErrorIndex() != 4)
                errln("setErrorIndex failed, got " + pos.getErrorIndex() + " instead of 4");
            if (objs != null) errln("unparsable string, should return null");
        }
        ...
        { // Taken from Test4118594().
            ...
            MessageFormat mf =
                    new MessageFormat("{" + argName + "}, {" + argName + "}, {" + argName + "}");
            String forParsing = "x, y, z";
        ...
        { // Taken from test4293229().
            MessageFormat format =
                    new MessageFormat("'''{'myNamedArgument}'' '''{myNamedArgument}'''");
            Map<String, Object> args = new HashMap<>();
            String expected = "'{myNamedArgument}' '{myNamedArgument}'";
            String result = format.format(args);
            if (!result.equals(expected)) {
                throw new RuntimeException(
                        "wrong format result - expected \""
                                + expected
                                + "\", got \""
                                + result
                                + "\"");
    ...
    public void TestSerialization() {
        ...
        assertEquals(
                "MessageFormats (empty pattern) before and after serialization are not equal",
                format1,
                format2);
        ...
        format2 = serializeAndDeserialize(format1);
        assertEquals(
                "MessageFormats (with custom formats) before and after serialization are not equal",
                format1,
                format2);
        assertEquals(
