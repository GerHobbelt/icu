...
FormattedNumberRange LocalizedNumberRangeFormatter::formatFormattableRange(
    ...
    LocalPointer<impl::UFormattedNumberRangeData> results(new impl::UFormattedNumberRangeData(), status);
    if (U_FAILURE(status)) {
...
LocalizedNumberRangeFormatter::getFormatter(UErrorCode& status) const {
    ...
    LocalPointer<impl::NumberRangeFormatterImpl> temp(new impl::NumberRangeFormatterImpl(fMacros, status), status);
    if (U_FAILURE(status)) {
        return nullptr;
    }
